...
| 通常表示 | マウスオーバー時 |
| --- | --- |
| ![ペットと週次残量を表す1本のリング](docs/assets/weekly-pet-rings-20260918.png) | ![ペット、週次残量80%とJSTのリセット日時](docs/assets/weekly-pet-hover-20260918.png) |

公開済みのペット画像と現行アプリのリング描画コードを使った、黒背景の表示プレビューです。新たに撮影した実画面のスクリーンショットではありません。残量とリセット日時はサンプル値です。文字サイズは初期値の `大`（2×）です。
